...
module.exports = {
    getAllTestBridgingPatients: async (req, res) => {
        const {
            ...
            is_validation,
            sync_status,
            patient_code = '' // Tambahkan parameter patient_code
        } = req.query;
        ...
        try {
            ...
            // Tambahkan filter sync_status jika ada
            if (sync_status !== undefined && sync_status !== '') {
                const validSyncStatuses = [0, 1, '0', '1'];
                if (!validSyncStatuses.includes(sync_status)) {
                    return res.status(400).json({
                        status: 'error',
                        statusCode: 400,
                        message: 'Invalid sync_status. Must be 0 or 1.',
                        data: null
                    });
                }
                filters.sync_status = parseInt(sync_status, 10);
            }
